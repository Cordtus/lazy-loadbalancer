...
let chainCounters: Record<string, number> = {};
let failureCounts: Record<string, number> = {};
const failureThreshold = 3;

const getChainData = async (chainName: string): Promise<ChainData | null> => {
  try {
    const response = await fetch(`${chainRegistryBaseUrl}${chainName}/chain.json`);
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    const data = await response.json() as ChainRegistryResponse; // Add type assertion
...
const updateChainData = async (chainName: string): Promise<ChainData | null> => {
...
const getChainEntry = async (chainName: string): Promise<ChainData | null> => {
  let chains: ChainsFile;
  try {
    chains = JSON.parse(fs.readFileSync(chainsFilePath, 'utf-8'));
  } catch (err) {
...
const getNextRpcAddress = (chainName: string, chainEntry: ChainData): string | null => {
...
const handleRpcFailure = (rpcUrl: string): void => {
...
app.get('/rpc-lb/:chainName', async (req: Request, res: Response) => {
...
app.all('/rpc-lb/:chainName/*', async (req: Request, res: Response) => {
  const chainName = req.params.chainName;
  const chainEntry = await getChainEntry(chainName);
