...
export function flushCache(pattern?: string): number {
  ...
  logger.info(`Flushed ${deletedCount} items from cache${pattern ? ` matching pattern ${pattern}` : ''}`);
  return deletedCount;
}
...
setInterval(() => {
  ...
  const totalKeys = 
    stats.main.keys + 
    stats.persistent.keys + 
    stats.session.keys + 
    stats.metrics.keys;
    
  const totalMemoryKB = 
    (stats.main.ksize + stats.main.vsize +
     stats.persistent.ksize + stats.persistent.vsize +
     stats.session.ksize + stats.session.vsize +
     stats.metrics.ksize + stats.metrics.vsize) / 1024;
  ...
  if (totalMemoryKB > 100000) { // 100MB threshold
